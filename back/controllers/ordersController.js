...
module.exports = {
    ...
    findByStatus(req,res){
        ...
        Order.findByStatus(status,(err,data)=>{
            ...
            for (const d of data) {
                if (typeof d.address === 'string' && typeof d.client === 'string' && typeof d.conductors === 'string') {
                    d.address = JSON.parse(d.address);
                    d.client = JSON.parse(d.client);
                    d.conductors = JSON.parse(d.conductors);
   ...
   async create(req, res) {
        ...
        Order.create(order, async (err, id) => {
            ...
            for (const conductor of order.conductor) {
                await OrderHasProducts.create(id,conductor.id,conductor.quantity,(err,id_data)=>{
                    if (err) {
                        return res.status(501).json({
                            success: false, //tipo de repuesta porque es una respuesta no exitosa
