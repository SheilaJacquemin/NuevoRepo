 ...
 async login(req, res) {
    ...
    User.findByEmail(email, async (err, myUser) => {
      ...
      if (isPasswordValid) {
        ...
        // Buscar los roles del usuario utilizando el modelo Rol.
        Rol.findUserRoles(myUser.id, async (rolesErr, roles) => {
          if (rolesErr) {
            return res.status(500).json({
              success: false,
              message: 'Hubo un error al obtener los roles del usuario',
              error: rolesErr,
            });
          }
          ...
          const data = {
            ...
            roles: roles, // Agrega los roles del usuario en la respuesta
          };
